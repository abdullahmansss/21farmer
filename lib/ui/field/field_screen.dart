import 'package:farmers21/ui/devices/gateways_screen.dart';
...
class _FieldScreenState extends State<FieldScreen>
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FieldBloc, FieldStates>(
      builder: (BuildContext context, FieldStates state) {
        FieldBody field = BlocProvider.of<FieldBloc>(context).field;
        if (state is ErrorFieldState) {
          showToast(state.error, false);
        }
        if (state is! FieldRefreshingState) {
          completer.complete();
        }
        return Scaffold(
          backgroundColor: Color(0xFFEEEEEE),
          appBar: buildAppBar(context, field),
          body: RefreshIndicator(
            onRefresh: () {
              if (state is! FieldLoadingState)
                BlocProvider.of<FieldBloc>(context)
                    .add(RefreshFieldEvent(testToken, field.id));
  ...
  Widget buildAppBar(BuildContext context, FieldBody field) {
    return AppBar(
